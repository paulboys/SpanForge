...
from tests.assertions import SpanAsserter, OverlapChecker
...
class TestLongTextHandling(IntegrationTestBase):
    ...
    def test_single_sentence_repeated_many_times(self):
        ...
        self.assertGreater(len(headache_spans), 100, 
                          "Should detect repeated symptom mentions")
...
class TestConcurrentDetection(IntegrationTestBase):
    ...
    def test_text_with_many_symptoms(self):
        ...
        self.assertGreater(len(symptom_spans), 3, 
                          "Should detect multiple symptom mentions")
        
        # Verify no conflicting overlaps
        span_dicts = [
            {"start": s.start, "end": s.end, "label": s.label}
            for s in symptom_spans
        ]
    ...
    def test_dense_span_coverage(self):
        ...
        self.assertGreater(coverage_ratio, 0.5, 
                          "Dense text should have >50% span coverage")
    ...
    def test_special_characters_at_scale(self):
        ...
        for spans in spans_batch:
            self.assertGreater(len(spans), 0, 
                             "Should detect spans despite special characters")
...
class TestPerformanceBenchmarks(IntegrationTestBase):
    ...
    def test_average_time_per_document(self):
        ...
        texts = [
            f"Patient {i} has headache after aspirin use."
            for i in range(100)
        ]
        ...
        self.assertLess(avg_time, 0.1, 
                       f"Avg time per doc: {avg_time*1000:.2f}ms (should be <100ms)")
    ...
    def test_lexicon_lookup_efficiency(self):
        ...
        self.assertLess(elapsed, 10.0, 
                       f"1000 iterations took {elapsed:.2f}s (should be <10s)")
