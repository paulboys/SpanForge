...
def serialize_spans(spans_batch: List[List]):
    ...
    for idx, spans in enumerate(spans_batch):
        ...
        records.append({"id": idx, "text": texts[idx], "weak_spans": out_spans})  # texts captured from outer scope
    return records


def main():
    parser = argparse.ArgumentParser(description="Export weak labels to JSONL")
    parser.add_argument("--input", required=True, help="Path to input text file (one complaint per line)")
    parser.add_argument("--output", required=True, help="Path to output JSONL file")
    parser.add_argument("--scorer", default="wratio", choices=["wratio", "jaccard"], help="Fuzzy scorer to use")
    ...
    spans_batch = weak_label_batch(texts, symptom_lex, product_lex, negation_window=args.negation_window, scorer=args.scorer)
