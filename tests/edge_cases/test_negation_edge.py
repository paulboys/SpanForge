...
class TestNegationEdgeCases(WeakLabelTestBase):
    ...
    def test_multiple_negation_cues(self):
        ...
        if "itching" in symptom_spans:
            self.assertTrue(symptom_spans["itching"].negated,
                          "Itching should be negated by 'No'")
        if "redness" in symptom_spans:
            self.assertTrue(symptom_spans["redness"].negated,
                          "Redness should be negated by 'No' or 'or'")
...
@pytest.mark.parametrize("text,symptom,should_negate", [
    ...
    ("Itching resolved yesterday, no itching today", "itching", None),  # Multiple mentions, mixed
])
...
def test_negation_window_distances(negation_cue, distance, should_negate):
    ...
    if symptom_spans:
        if should_negate:
            assert symptom_spans[0].negated, \
                f"Expected negation at distance {distance}"
        else:
            assert not symptom_spans[0].negated, \
                f"Expected NO negation at distance {distance}"
...
class TestNegationOverlap(WeakLabelTestBase):
    ...
    def test_partial_negation_overlap(self):
        ...
        for span in symptom_spans:
            self.assertTrue(span.negated,
                          f"Symptom '{span.text}' should be negated by 'No'")
