...
class SpanAsserter:
    ...
    def assert_span_equals(self, expected: Dict[str, Any], actual: Dict[str, Any],
        ...
        """
        self.tc.assertEqual(expected['start'], actual['start'], 
                           f"Span start mismatch: {expected} vs {actual}")
        self.tc.assertEqual(expected['end'], actual['end'],
                           f"Span end mismatch: {expected} vs {actual}")
        self.tc.assertEqual(expected['label'], actual['label'],
                           f"Span label mismatch: {expected} vs {actual}")
        
        if 'canonical' in expected:
            self.tc.assertEqual(expected['canonical'], actual.get('canonical'),
                               f"Canonical mismatch: {expected} vs {actual}")
        
        if check_confidence and 'confidence' in expected:
            self.tc.assertAlmostEqual(
                expected['confidence'], actual.get('confidence', 1.0),
                places=2, msg=f"Confidence mismatch: {expected} vs {actual}"
            )
    
    def assert_span_list_contains(self, expected_span: Dict[str, Any],
    ...
    def assert_text_slices_match(self, text: str, spans: List[Dict[str, Any]]):
        ...
        for span in spans:
            ...
            self.tc.assertEqual(expected_text, span['text'],
                               f"Text slice mismatch at [{start}:{end}]")
    ...
    def assert_no_conflicting_labels(self, spans: List[Dict[str, Any]]):
        ...
        for i, span_a in enumerate(spans):
            for span_b in spans[i+1:]:
                ...
                if overlap > 0:
                    self.tc.assertEqual(
                        span_a['label'], span_b['label'],
                        f"Conflicting labels on overlapping spans: {span_a} vs {span_b}"
                    )
    
    def assert_overlap_matrix_valid(self, spans: List[Dict[str, Any]],
                                   allow_same_label_overlap: bool = True):
