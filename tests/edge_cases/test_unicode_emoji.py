...
class TestUnicodeHandling(WeakLabelTestBase):
    ...
    def test_accented_characters(self):
        ...
        self.assertTrue(
            any("itching" in t for t in symptom_texts),
    ...
    def test_cjk_characters(self):
        ...
        if spans:
            for span in spans:
                self.span_asserter.assert_boundaries_valid(
                    text, [{"start": span.start, "end": span.end, "label": span.label}]
                )
    ...
    def test_combining_marks(self):
        ...
        spans = [
            {
                "start": 0,
                "end": len(base_text_with_combining),
                "label": "SYMPTOM",
                "text": base_text_with_combining,
            }
        ]
...
def test_unicode_symptom_detection(text, expected_symptoms):
    ...
    for expected in expected_symptoms:
        assert any(
            expected in t for t in symptom_texts
