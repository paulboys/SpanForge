...
def plot_iou_uplift(report: Dict[str, Any], output_dir: Path, formats: List[str], dpi: int) -> None:
    ...
    if confidence_strata:
        ...
        for i, (w, l) in enumerate(zip(weak_ious, llm_ious)):
            if l > w:
                ax.annotate(f'+{l-w:.2f}', xy=(i, max(w, l) + 0.02), 
...
def plot_calibration_curve(report: Dict[str, Any], output_dir: Path, formats: List[str], dpi: int) -> None:
    """
    ...
    scatter = ax.scatter(mean_confidence, mean_iou, s=[c*20 for c in counts], 
...
def plot_correction_rate(report: Dict[str, Any], output_dir: Path, formats: List[str], dpi: int) -> None:
    """
...
def plot_prf_comparison(report: Dict[str, Any], output_dir: Path, formats: List[str], dpi: int) -> None:
    """
    ...
    llm_values = [
        llm_prf.get('precision', 0.0),
        llm_prf.get('recall', 0.0),
        llm_prf.get('f1', 0.0)
    ]
    ...
    for i, (w, l) in enumerate(zip(weak_values, llm_values)):
        if abs(l - w) > 0.001:
            ...
            ax.annotate(f'{sign}{delta:.3f}', xy=(i, max(w, l) + 0.08), 
...
def plot_stratified_comparison(report: Dict[str, Any], output_dir: Path, formats: List[str], dpi: int) -> None:
    """
    ...
    if confidence_strata:
        ...
        for i, (delta, count) in enumerate(zip(iou_deltas, counts)):
            ax.text(i, delta + (0.02 if delta >= 0 else -0.02), 
                   f'{delta:+.3f}\n(n={count})', 
