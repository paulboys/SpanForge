        ...
        spans = [
            {
                ...
                "end": 25,
                "label": "SYMPTOM",
                "text": "itching and burn",
                "canonical": "pruritus",
            },
            {"start": 20, "end": 30, "label": "SYMPTOM", "text": "burning", "canonical": "burning"},
        ]
...
@pytest.mark.parametrize(
    "span_a,span_b,expected_overlap,expected_conflict",
    [
...
@pytest.mark.parametrize(
    "iou_threshold,span_a,span_b,should_match",
    [
...
def test_iou_thresholds(iou_threshold, span_a, span_b, should_match):
    ...
    if should_match:
        assert iou >= iou_threshold, f"Expected IOU >= {iou_threshold}, got {iou}"
    else:
        assert iou < iou_threshold, f"Expected IOU < {iou_threshold}, got {iou}"
