...
import unittest
import json
from tests.base import WeakLabelTestBase, IntegrationTestBase
from tests.assertions import SpanAsserter, IntegrityValidator
...
@pytest.mark.parametrize("record,expected_error", [
    ...
    ({"text": "Sample", "entities": "not_a_list", "source": "test"}, (TypeError, AssertionError)),
...
@pytest.mark.parametrize("text,label,should_fail", [
...
def test_span_label_validation(text, label, should_fail):
    ...
    entities = [{"start": 0, "end": len(text), "label": label, 
                "text": text, "canonical": "test"}]
